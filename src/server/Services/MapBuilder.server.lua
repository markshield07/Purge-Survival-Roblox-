...
local function CreateHouse(position: Vector3, zone: number, houseIndex: number, isStarter: boolean?)
	...
	-- Walls (with doorway gaps in front and back)
	local doorWidth = 4
	local doorHeight = 7
	local sideWidth = (w - doorWidth) / 2  -- width of each wall segment beside door

	-- Front wall: split into left, right, and header above door
	CreatePart({
		Name = "WallFrontLeft",
		Size = Vector3.new(sideWidth, h, 1),
		Position = position + Vector3.new(-(sideWidth / 2 + doorWidth / 2), h / 2 + 0.5, -d / 2),
	...
	CreatePart({
		...
		Position = position + Vector3.new(0, doorHeight + (h - doorHeight) / 2 + 0.5, -d / 2),
		Material = Materials.HouseWall.material,
		Color = wallColor,
		Parent = houseFolder,
	})

	-- Back wall: split into left, right, and header above door
	CreatePart({
		Name = "WallBackLeft",
		Size = Vector3.new(sideWidth, h, 1),
		Position = position + Vector3.new(-(sideWidth / 2 + doorWidth / 2), h / 2 + 0.5, d / 2),
	...
	-- Side walls (solid, no doors)
	CreatePart({
		Name = "WallLeft",
		Size = Vector3.new(1, h, d),
		Position = position + Vector3.new(-w / 2, h / 2 + 0.5, 0),
		Material = Materials.HouseWall.material,
		Color = wallColor,
		Parent = houseFolder,
	})
	...
	-- Front door (walk-through by default, becomes solid when fortified)
	...
	-- Back door (walk-through by default, becomes solid when fortified)
